...
function getPolynom(...args) {
  return (x) => {
    let exp = args.length;
    return args.reduce((acc, next, index, array) => {
      if (index === array.length - 1) {
        return acc + next;
      }
      exp -= 1;
      return acc + (next * x ** exp);
    }, 0);
  };
}
...
function logger(func, logFunc) {
  return (...arg) => {
    const argResult = JSON.stringify(arg).slice(1, -1);
    logFunc(`${func.name}(${argResult}) starts`);
    const result = func(...arg);
    logFunc(`${func.name}(${argResult}) ends`);
    return result;
  };
}
